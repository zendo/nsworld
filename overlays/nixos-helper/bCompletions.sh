...
#COMPREPLY=(which run boot switch upgrade hmswitch hmsource hmdiff diff source installed profiles generations references depends fetch-merge run-pr pull-pr index-update)
  ...
  # suggestions words
  local suggestions=($(compgen -W "which log run shell search boot switch upgrade wslswitch hmswitch hmsource hmdiff diff source installed profiles generations references depends gitfm pr-run pr-shell pr-pull index-update" -- "${COMP_WORDS[1]}"))
