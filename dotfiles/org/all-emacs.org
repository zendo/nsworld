...
(when (boundp 'w32-get-true-file-attributes)
  (setq w32-get-true-file-attributes nil ; decrease file IO workload
        ...
        w32-pipe-buffer-size (* 64 1024) ; read more at a time
        ))
...
;; (setq-default frame-title-format "%b (%f)") ;标题栏显示正在编辑的文件名和路径
...
(when IS-WINDOWS
  (setq default-directory "C:/Users/zendo/Desktop/" ;主目录
        ...
        package-check-signature nil ; 禁用elpa包签名验证
        user-lisp-auto-scrape nil ; 禁用自动扫描lisp等用户目录
        native-comp-deferred-compilation nil ; 禁用后台延迟编译
        projectile-git-submodule-command nil
        )
  (set-selection-coding-system 'utf-16le-dos) ; encoding
  (set-frame-font (font-spec :family "JetBrains Mono" :size 28))
  (dolist (script '(han cjk-misc bopomofo))
    (set-fontset-font
     (frame-parameter nil 'font)
     script
...
(add-to-list 'package-pinned-packages '(ghostel . nil))
#+end_src
...
*** shell-pop
https://www.jamescherti.com/emacs-toggle-a-shell-window-shell-pop/
#+begin_src emacs-lispxx
(use-package shell-pop
  :bind (("C-`" . shell-pop))
  :custom
  ;; (shell-pop-universal-key "C-c t") ; did't work?
  (shell-pop-window-size 30)
  (shell-pop-full-span nil) ; Don't stretches across the entire frame width
  ;; (shell-pop-term-shell shell-file-name)
  (shell-pop-shell-type
        (cond ((eq system-type 'gnu/linux) '("ghostel" "*ghostel*" #'ghostel))
...
;; bug: 会导致 magit commit save 后跳到 dashboard
...
(use-package occult
  :vc (:url "https://github.com/agzam/occult.el"
            :rev :newest)
  :bind (("C-c z" . occult-toggle)
         ("C-c Z" . occult-reveal-all))
  )
#+end_src
...
* Persistence & History
** auto-save-mode #filename#
#+begin_src emacs-lisp
;; 传统备份机制，不覆盖原文件，只生成一个临时备份文件。
;; Enable `auto-save-mode' to prevent data loss. Use `recover-file' or
;; `recover-session' to restore unsaved changes. #file.txt#
...
;; (setq auto-save-visited-interval 3) ; Save after 5 seconds if inactivity
...
(use-package super-save
  ...
  (super-save-idle-duration 3) ; default: 5
...
** buffer-guardian
https://www.jamescherti.com/buffer-guardian-el-automatically-save-emacs-buffers/
#+begin_src emacs-lisp
...
(use-package consult
  ...
  :bind (;; C-c bindings in `mode-specific-map'
         ...
         ("C-x C-b" . consult-buffer)              ;; orig. list-buffer
...
(defun my/prog-mode-defaults ()
  "Default behaviors shared by programming modes."
  (hl-line-mode 1) ;高亮当前行
  (whitespace-mode 1) ;显示空格
  (display-line-numbers-mode 1) ;显示行号
  (setq-local word-wrap nil) ;在任意字符处拆分单词换行
  (setq-local truncate-lines nil)) ;启用行截断
...
;; (use-package direnv
;;   :when (eq system-type 'gnu/linux) ; Windows unsupported
;;   :config
;;   (direnv-mode))
#+end_src
...
(use-package eglot
  ...
  (eglot-code-action-indications nil) ; Disable automatic code action indicators
  :hook ((prog-mode . eglot-ensure))
  :config
  ;; Disable live formatting
...
(use-package anju
  :init
  (context-menu-mode 1) ; 全局开启 Emacs 的右键菜单基础模式
  :config
  (anju-init))
#+end_src
